...
class UpChunk {
  ...
  UpChunk({
    ...
    var checkEndpoint =
        connectionCheckEndpoint != null
            ? connectionCheckEndpoint!
            : _endPointUri.origin;
    _internetConnection = InternetConnection.createInstance(
      customCheckOptions: [InternetCheckOption(uri: Uri.parse(checkEndpoint))],
  ...
  void stop() {
    ...
    if (onError != null) {
      onError!('Upload cancelled by the user.', _chunkCount, _attemptCount);
    }
  }
  ...
  Future<Response> _sendChunk() async {
    ...
    if (onAttempt != null) onAttempt!(_chunkCount, _chunkLength);
